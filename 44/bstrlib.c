...
int balloc(bstring b, int olen)
{
	...
	if (olen >= b->mlen)
	{
		...
		else
		{
			...
			{
				if (b->slen)
					bstr__memcpy((char *)x, (char *)b->data,
								 (size_t)b->slen);
				bstr__free(b->data);
			}
		}
...
int bcatcstr(bstring b, const char *s)
{
	...
	if (b == NULL || b->data == NULL || b->slen < 0 || b->mlen < b->slen || b->mlen <= 0 || s == NULL)
		return BSTR_ERR;
...
int bcatblk(bstring b, const void *s, int len)
{
	...
	if (b == NULL || b->data == NULL || b->slen < 0 || b->mlen < b->slen || b->mlen <= 0 || s == NULL || len < 0)
		return BSTR_ERR;
			 ...
			 unsigned char fill)
{
	...
	if (aux->slen != len)
		bstr__memmove(b1->data + pos + aux->slen,
					  b1->data + pos + len,
					  b1->slen - (pos + len));
...
int bsreada(bstring r, struct bStream *s, int n)
{
	...
	if (s == NULL || s->buff == NULL || r == NULL || r->mlen <= 0 || r->slen < 0 || r->mlen < r->slen || n <= 0)
		return BSTR_ERR;
...
{
	if (s == NULL || s->buff == NULL || r == NULL || term == NULL || term->data == NULL || r->mlen <= 0)
		return BSTR_ERR;
	if (term->slen == 1)
				 ...
				 int (*cb)(void *parm, int ofs, const_bstring entry), void *parm)
{
	...
	if (cb == NULL || s == NULL || s->readFnPtr == NULL || splitStr == NULL || splitStr->slen < 0)
		return BSTR_ERR;
			  ...
			  int (*cb)(void *parm, int ofs, int len), void *parm)
{
	...
	if (cb == NULL || str == NULL || pos < 0 || pos > str->slen || splitStr == NULL || splitStr->slen < 0)
		return BSTR_ERR;
	if (splitStr->slen == 0)
	{
...
int bformata(bstring b, const char *fmt, ...)
{
	...
	if (b == NULL || fmt == NULL || b->data == NULL || b->mlen <= 0 || b->slen < 0 || b->slen > b->mlen)
		return BSTR_ERR;
...
int bvcformata(bstring b, int count, const char *fmt, va_list arg)
{
	...
	if (b == NULL || fmt == NULL || count <= 0 || b->data == NULL || b->mlen <= 0 || b->slen < 0 || b->slen > b->mlen)
		return BSTR_ERR;
